...
def main(name, spec, times, freqs):
    ...
    for i in index_freqs:
        ...
        for j in range(len(spec[0])):
            ...
            if dBtemp > spec[i[0]][j] > dBthreshold and dBpasttemp < dBtemp > dBthreshold and temp_time == 0:
                song.append(('note_on', 0, itemp + 24, 100, int((jtemp-1) * times[1] * 1000)))
                ...
                song.append(('note_on', 0, itemp + 24, 0, int((jtemp-2) * times[1] * 1000)))
                song.append(('note_on', 0, itemp + 24, 100, int((jtemp-1) * times[1] * 1000)))
                list_of_keys.append((i[0], (jtemp - temp_time) * times[1], temp_time * times[1]))
                temp_time = 1
                #print("VER2")
                ...
                temp_time += 1
                #print("VER3")
            elif spec[i[0]][j] <= dBthreshold/1.2 and temp_time > 0:
                song.append(('note_on', 0, itemp + 24, 0, int(jtemp * times[1] * 1000)))
                list_of_keys.append((i[0], (jtemp - temp_time) * times[1], temp_time * times[1]))
                temp_time = 0
                #print("VER4")
            dBpasttemp = dBtemp
    ...
    current_directory = os.path.dirname(os.path.abspath(file))  # Ścieżka bieżącego folderu
