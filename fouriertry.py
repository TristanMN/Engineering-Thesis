...
librosa.display.specshow(np.power(np.minimum(D_db,31),2)/np.power(int(np.minimum(D_db,31).max()),1), x_axis='time', y_axis='log', sr=sr, cmap='viridis', n_fft = n_fft, hop_length = hop_length)
